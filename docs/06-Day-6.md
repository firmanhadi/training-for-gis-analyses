...
Layer hillshade umumunya digunakan untuk memperbagus tampilan peta dan tampilan topografi yang intuitif, dengan mensimulasikan sumber cahaya dan bayangan yang dibentuk oleh permukaan bumi. Hillshade dapat dihitung dengan cara sebagai berikut :
...
Seringkali kita harus bekerja dengan data besar dan padat fiturnya, yang membuat penampilan datanya lambat. Jumlahnya yang ribuan atau bahkan jutaan fitur seringkali sulit diinterpretasi, overlap antar fitur menyebabkan pendeteksian pola cluster atau distribusinya tidak mudah untuk dilakukan.   

Pada bagian ini, kita akan belajar mengenai teknik yang memungkinkan visualisasi dataset semacam itu dalam bentuk yang lebih mudah dibaca dan dengan waktu pemuatan (loading) yang lebih cepat. Setelah melakukan praktek di bagian ini, Anda diharapkan mampu melakukan analisis kepadatan (density analyses) untuk data yang Anda miliki dan mengekstrak informasi dari peta kepadatan.
...
Peta kerapatan memberi kita kemampuan untuk memperkirakan konsentrasi beberapa fitur dalam suatu area. Ini membantu kita menemukan area di mana reaksi mendesak diperlukan atau yang cocok dengan kriteria Anda. Heatmaps juga membantu mengontrol kondisi dan perubahannya.
...
Kasus penggunaan yang paling umum adalah pembuatan peta kerapatan untuk menampilkan kerapatan fitur titik. Peta seperti ini sering disebut peta panas. Apa itu peta panas? Ini adalah layer raster. Setiap selnya berisi representasi kepadatan fitur di sekitarnya (misalnya, jumlah orang per kilometer persegi), yang tergantung pada jumlah fitur dalam beberapa area.

Untuk membuat peta panas, dalam kasus paling sederhana, GIS melihat fitur di sekitar pusat sel, menggunakan radius pencarian yang diberikan. Kemudian jumlah fitur yang termasuk dalam radius yang diberikan dihitung dan dibagi dengan luas wilayah. Nilai ini akan menjadi nilai sel. Kemudian sel selanjutnya akan dianalisis, dan seterusnya. Sebagai hasilnya, kita akan mendapatkan kombinasi nilai, yang menciptakan permukaan yang halus. Untuk memahami ini lebih baik, lihat diagram berikut:
...
Diagram ini menunjukkan prinsip umum pembuatan **_heatmap_**. Titik-titik hijau menggambarkan fitur yang digunakan untuk pembuatan peta kerapatan, kotak biru adalah sel raster saat ini, dan lingkaran titik-titik merah menandai radius pencarian, misalnya, 1 km. Dalam hal ini, area yang dicakupi adalah sekitar 3,14 km persegi. Seperti yang dapat kita lihat pada diagram di sebelah kiri, empat fitur berada dalam radius pencarian. Jadi, sel raster akan mendapatkan nilai 4 / 3.14 = 1.27. Di sisi kanan, kita perhatikan bahwa sel berikutnya akan mendapatkan nilai 1,59 karena sekarang ada lima fitur di dalam radius pencarian.

Ini adalah pendekatan paling sederhana. Dalam aplikasi dunia nyata, algoritma yang lebih kompleks digunakan, di mana setiap titik memiliki dampak pada nilai-nilai sel tetangga, tergantung pada jaraknya dari sel-sel tersebut.
...
Informasi tentang tempat-tempat seperti itu mungkin berguna bagi departemen kepolisian atau lembaga lain untuk merencanakan beberapa kegiatan untuk mengurangi kebisingan, atau bagi mereka yang mencari apartemen dan tidak ingin tetangga yang hobinya mendengarkan musik cadas .
...
2. Pilih layer __noise__ dari kotak kombo __Input point layer__.
...
4. Gunakan kotak kombo __Output format__ untuk memilih format data yang diinginkan. Pilihan paling umum adalah __GeoTIFF__, namun untuk peta yang mencakup area luas, lebih baik gunakan format yang lain, misalnya __Erdas Imagine__.
...
Untuk mengatur hasil yang lebih baik, kita dapat memilih kotak__Advanced__ box dan menentukan beberapa parameter tambahan:
...
Ukuran sel raster menentukan seberapa kasar atau terperinci tampilan pola distribusi. Ukuran sel yang lebih kecil akan memberikan hasil yang lebih halus, tetapi waktu pemrosesan dan memori yang diperlukan untuk analisis akan meningkat. Sel besar akan diproses lebih cepat, tetapi raster yang dihasilkan akan pixelated. Jika sel-selnya sangat besar, beberapa pola akan menjadi tidak terlihat, jadi Anda mungkin perlu menjalankan analisis beberapa kali, mencoba ukuran sel yang berbeda untuk mendapatkan hasil yang memenuhi kebutuhan Anda.

Ukuran sel tergantung pada dan terkait dengan dimensi raster. Menambahnya akan mengurangi jumlah baris dan kolom, dan sebaliknya.

- __Kernel shape__: 

Ini mengontrol bagaimana titik memengaruhi perubahan dengan perubahan jarak dari titik ini. __Heatmap plugin__ QGIS saat ini mendukung kernel seperti berikut:
...
Bergantung pada bentuk kernel, kita akan mendapatkan peta panas yang lebih halus, atau hotspot yang lebih jelas. Misalnya, kernel triweight akan memberikan hotspot yang lebih jelas, lebih tajam daripada kernel Epanechnikov, karena kernel Epanechnikov memiliki pengaruh lebih rendah di dekat pusat hotspot. Juga, dalam bidang ilmiah yang berbeda, kernel yang berbeda lebih disukai; misalnya, dalam analisis kejahatan, kernel kuartik biasanya digunakan.

Dimungkinkan juga untuk menggunakan jari-jari pencarian variabel untuk setiap titik dengan memilih kotak centang Gunakan jari-jari dari bidang dan memilih bidang atribut dengan nilai jari-jari dari kotak kombo. Jika Anda perlu menimbang poin (dengan kata lain, menambah atau mengurangi pengaruhnya) dengan beberapa atribut numerik, aktifkan kotak centang Gunakan bobot dari bidang dan pilih bidang yang sesuai. Dalam contoh kita, kita tidak akan menggunakan fungsi ini, tetapi Anda dapat mencobanya sendiri.

Seperti yang kita katakan sebelumnya, ukuran sel memiliki pengaruh langsung pada kualitas peta panas yang dihasilkan, jadi penting untuk memilihnya dengan hati-hati. Dalam kebanyakan kasus, ukuran sel dipilih sedemikian rupa sehingga kita mendapatkan 10 hingga 100 sel per unit area (yang pada gilirannya ditentukan oleh radius pencarian). Untuk menghitung ukuran sel, kita perlu menyelaraskan unit area dengan unit jarak; misalnya, jika kita menghitung kepadatan menggunakan kilometer persegi dan menentukan radius pencarian dalam meter, maka perlu untuk mengubah kilometer persegi menjadi meter persegi. Langkah selanjutnya adalah membagi area dengan jumlah sel yang diinginkan. Akhirnya, karena ukuran sel ditentukan oleh lebar atau tingginya (karena sel raster biasanya memiliki bentuk persegi), kita perlu mengekstrak akar kuadrat dari nilai ini.

Dalam contoh kita, kita akan membuat peta panas dengan radius pencarian 1000 m, sehingga area pencarian akan sekitar 3,14 kilometer persegi. Saat dinyatakan dalam meter, ini akan menjadi sebagai berikut:

\begin{equation*}
3.14 km^2 = 3.14 . 1000 m . 1000 m = 3140000m^2
\end{equation*}

Karena kita ingin peta panas yang halus, kita akan menggunakan jumlah piksel yang relatif besar per satuan luas; katakanlah 100 piksel per 3,14 kilometer persegi. Jadi, kita membagi area dalam meter persegi dengan jumlah piksel yang diinginkan:
...
Tentu saja ini bukan aturan ketat tapi hanya rekomendasi. Anda dapat menggunakan ukuran sel lain dengan aman, tergantung pada data Anda dan hasil yang diinginkan. Hanya ingat bahwa nilai yang lebih kecil mengarah pada peta panas yang lebih halus, tetapi pada saat yang sama meningkatkan waktu analisis dan menghasilkan ukuran raster.

Ketika semua input dan parameter diatur, tekan tombol OK untuk memulai proses pembuatan peta panas. Kemajuan pembentukan peta panas akan ditampilkan dalam dialog progres kecil. Jika proses ini terlalu lama untuk diselesaikan, Anda dapat menghentikannya dengan menekan tombol Batalkan. Perhatikan bahwa setelah membatalkan pembuatan peta panas, Anda masih mendapatkan hasilnya, tetapi hasilnya tidak lengkap dan tidak berguna untuk analisis lebih lanjut.

Ketika proses selesai, peta panas yang dihasilkan akan ditambahkan ke QGIS sebagai raster grayscale, di mana wilayah yang lebih terang sesuai dengan nilai kepadatan yang lebih tinggi dan daerah yang lebih gelap sesuai dengan nilai kepadatan yang lebih rendah, seperti ini:
...
Untuk meningkatkan keterbacaan dan membuatnya terlihat seperti peta panas nyata, kita perlu mengubah gayanya. Untuk melakukan ini, ikuti langkah selanjutnya.
...
Sekarang kita dapat dengan mudah menemukan titik terpanas (ditampilkan dalam warna lebih dekat ke merah jika peta warna Merah digunakan), dan bahkan mengenali beberapa pola distribusi yang tidak terlihat ketika kita melihat lapisan titik asli:
...
Sekarang kita dapat dengan mudah menemukan titik terpanas (ditampilkan dalam warna lebih dekat ke merah jika peta warna Merah digunakan), dan bahkan mengenali beberapa pola distribusi yang tidak terlihat ketika kita melihat lapisan titik asli. Juga, lapisan peta panas kita muncul jauh lebih cepat daripada vektor yang digunakan untuk membuat peta panas ini.


__Mendeteksi wilayah terpanas ("hottest")__

Terkadang, Anda tidak perlu peta panas itu sendiri, tetapi hanya ingin menemukan hotspot — area dengan kepadatan tertinggi — dan menggunakannya dalam analisis lebih lanjut. Sangat mudah untuk menemukan wilayah seperti itu di QGIS dan mengekstraknya dalam bentuk vektor.

Pertama, kita harus mendefinisikan nilai ambang, yang akan digunakan untuk mengenali hotspot. Sebagai nilai awal, kita dapat menggunakan nilai piksel maksimum dalam peta panas kita dan kemudian menyesuaikannya sesuai kebutuhan kita.
...
Untuk mengonversi topeng raster ke dalam format vektor, kita perlu membuat poligon dari semua piksel yang terhubung dengan nilai yang sama. Di sinilah alat Polygonize datang untuk membantu. Di kotak alat Pemrosesan, Anda dapat menemukan algoritma Polygonize dengan mengetik namanya di bidang filter di bagian atas kotak alat. Klik dua kali pada nama algoritma untuk membuka dialognya, dan Anda akan melihat sesuatu seperti ini:
...
Untuk menghapus fitur yang tidak perlu dari lapisan hotspot, pilih di pohon lapisan QGIS, klik kanan untuk membuka menu konteks, dan pilih Buka Tabel Atribut . Klik pada fitur Pilih menggunakan tombol ekspresi . Dalam dialog Pilih dengan ekspresi , masukkan “DN” = 0 (jika perlu, ganti DN dengan nama bidang Anda), klik tombol Pilih , dan tutup dialog. Mulai mengedit dengan mengklik Beralih mengedit tombol mode, atau tekan Ctrl + E . Untuk menghapus fitur yang dipilih, tekan tombol Hapus atau klik Hapus fitur yang dipilih . Terakhir, matikan mode pengeditan dengan menekan Ctrl + E atau mengklikBeralih mode edit lagi.
...
Selain mendeteksi titik panas, peta panas juga dapat digunakan untuk mendeteksi perubahan intensitas atau memvisualisasikan arah perubahan nilai. Cara paling umum untuk melakukan kedua tugas ini adalah pembuatan garis kontur.

Untungnya, QGIS memiliki semua alat yang diperlukan untuk ini. kita akan menggunakan pemrosesan lagi, tetapi pembuatan garis kontur juga tersedia di plugin GDALTools (yang dapat ditemukan di menu Raster ). Di kotak alat Pemrosesan , Anda dapat menemukan algoritma Kontur dengan mengetik namanya di bidang filter di bagian atas kotak alat. Klik dua kali pada nama algoritma untuk membuka dialognya, yang terlihat seperti ini:
...
Pilih heatmap lapisan raster sebagai masukan lapisan. Di bidang File output , tentukan jalur tempat hasil akan disimpan. Juga, perlu untuk menentukan Interval antara garis kontur . Tidak ada prinsip ketat tentang penentuan interval ini. Aturan umum adalah memilih interval yang mendeteksi pola di area dengan perubahan kepadatan halus. kita akan memilih interval 10.

Ketika semua informasi yang diperlukan telah ditentukan, klik Jalankan untuk memulai pembuatan garis kontur. Setelah beberapa waktu, layer vektor poligonal baru akan ditambahkan ke QGIS, dan kita dapat mulai menganalisisnya. Pertama, jika perlu, pindahkan layer kontur ke atas peta panas di pohon layer QGIS. Selain itu, lebih baik menyesuaikan simbologi kontur untuk membuatnya lebih mudah dikenali dari latar belakang peta panas.
...
Kontur yang lebih padat sesuai dengan perubahan kepadatan yang lebih intens. Selain itu, kita dapat mengidentifikasi arah perubahan kebisingan. Misalnya, dalam tangkapan layar sebelumnya, kita dapat melihat bahwa di beberapa tempat, distribusi kebisingan di sekitar pusat tidak sama; intensitas berkurang lebih cepat di tenggara daripada di barat laut. Jadi, kita dapat berasumsi bahwa ada beberapa hambatan untuk kebisingan di sana.
...
Apa pun pendekatan yang akan Anda ikuti, alur kerja analisis kesesuaian umum melibatkan beberapa langkah umum. kita sekarang akan melihat lebih dekat pada mereka untuk memastikan pemahaman yang lebih baik tentang sistem analisis kesesuaian:
...
Pertanian : Kesesuaian area untuk budidaya tanaman tertentu dinilai.

Ritel : Area dinilai dari calon pemasaran — apakah akan menarik pelanggan atau pembeli baru, atau tidak. Jenis analisis ini sangat diminati ketika memilih lokasi belanja yang disukai.
...
Relevansi data dengan tujuan dan sasaran didefinisikan. Ketersediaan data saat ini dan kebutuhan data masa depan dianalisis, terutama pengetahuan tentang apakah turunan data saat ini dapat digunakan untuk analisis atau tidak. Misalnya, jika kita harus menganalisis kesesuaian untuk kebutuhan pertanian, DEM dapat menjadi sumber yang bagus. Ini tidak hanya memberikan informasi dasar tentang bantuan, tetapi juga beberapa turunan yang bermanfaat, seperti kemiringan dan aspek. Hasil utama dari tahap ini adalah daftar sumber data primer dan turunan potensial mereka.
...
Segera setelah kita menetapkan persyaratan dasar, kita perlu menjelajahi lapisan data potensial untuk digunakan dan relevan untuk analisis. Dataset pelatihan berisi sejumlah besar dataset, dan yang paling relevan di antaranya tercantum dalam daftar berikut:
...
Raster yang dibuat di bagian Membuat peta panas dengan bagian plugin Heatmap, Menjawab Pertanyaan dengan Analisis Densitas, yang menunjukkan kerapatan spasial pengaduan kebisingan yang terdaftar mungkin berguna untuk penilaian potensial untuk keselamatan publik.
...
Lapisan yang berisi fitur ruang terbuka, seperti lapangan, trek, taman, dan sebagainya.
...
Menurut uraian hurricane_evacuation_zones, ada enam zona, yang diperingkat dalam zona bidang atribut berdasarkan risiko dampak gelombang badai, dengan zona 1 menjadi wilayah yang kemungkinan besar akan banjir. Jika terjadi badai atau badai tropis, penghuni di zona ini dapat diperintahkan untuk mengungsi. Daerah dengan nilai zona X tidak berada dalam zona evakuasi. Area dengan nilai zona 0 adalah salah satu dari yang berikut: air, dermaga kecil, atau pulau-pulau tak berpenghuni. Untuk keperluan analisis, lapisan ini harus dirasterisasi dan diberi peringkat sesuai dengan risiko dampak badai, dengan nilai peringkat turun dari area yang tidak berada dalam zona evakuasi ke yang paling mungkin terkena banjir.

Lapisan poligon hurricane_inundation_zones berisi informasi tentang risiko genangan lonjakan badai di bidang atribut kategori, di mana nilainya adalah ketinggian lonjakan kaki. Area yang kemungkinan besar akan tergenang diberi nilai 1, dan area yang dikecualikan dari pemodelan inundasi diberi nilai 5. Lapisan ini harus dirasterisasi dan diberi peringkat dengan nilai kesesuaian potensial tertinggi untuk area yang dikecualikan, dan yang terendah untuk bidang kategori 1.

Layer raster noise_heatmap adalah raster yang harus diberi peringkat menggunakan beberapa kategori, dengan nilai kesesuaian terendah untuk tempat paling berisik dan sebaliknya. Hal yang baik di sini adalah kita tidak perlu meraster layer ini, seperti yang kita lakukan pada layer sebelumnya. Pada saat yang sama, menetapkan jumlah dan kisaran untuk peringkat membawa subjektivitas ke dalam penilaian kita. Lapisan tree_density, yang juga merupakan raster kepadatan, harus dianalisis dengan cara yang sama.

Lapisan yang dipilih lainnya harus dianalisis terlebih dahulu untuk kedekatannya. Untuk tujuan ini, pertama-tama kita akan merasterisasi mereka, kemudian membuat raster proximity terus menerus, dan akhirnya peringkat mereka di bawah beberapa kategori sesuai dengan nilai proximity (semakin dekat suatu objek, semakin tinggi nilai kesesuaian). Sekali lagi, dalam hal peringkat pengguna, kita tidak akan dapat menghindari beberapa subjektivitas dalam penilaian kita. Selain itu, raster kedekatan akhir dapat ditimbang menurut kepentingannya dalam penilaian kesesuaian keseluruhan.
...
- **Raster kepadatan peringkat** 

Ini adalah peta panas raster yang harus dikonversi dari cakupan terus-menerus ke nilai yang dikategorikan di mana nilai tertinggi melambangkan area yang paling tepat, dan yang terendah terkait dengan area yang paling tidak cocok. Contoh dari layer ini adalah noise_heatmap dan tree_density.

- **Menghasilkan dan menentukan peringkat raster proximity**

Ini adalah alur kerja yang paling membosankan. Lapisan vektor harus diraster terlebih dahulu, dan kemudian raster proximity harus dibuat dan diberi peringkat dengan benar. Kategori ini mencakup lapisan vektor berikut: subway_entrances, taman, public_schools, bike_routes, athletic_facilities, dan museumart.
...
Dalam contoh ini, kita akan mengerjakan lapisan hurricane_evacuation_zones. Atribut yang sangat kita minati adalah zona. Ini karena dengan atribut ini, daerah diprioritaskan untuk evakuasi. Daerah dengan nilai terendah, 1, kemungkinan besar akan dievakuasi, dan sebaliknya. Dalam hal ini, kita dapat menggunakan nilai-nilai ini secara langsung untuk memberi peringkat raster.

Satu hal yang mencegah kita melakukan rasterisasi secara langsung adalah bahwa bidang atribut yang digunakan untuk rasterisasi harus berupa angka. Jika Anda memeriksa bidang atribut di bagian Bidang di bawah Properti, Anda akan melihat bahwa zona bidang memiliki Tipe QString, dan nilainya yang berisi angka 1-6 dan huruf (X) ditafsirkan bukan sebagai angka tetapi sebagai urutan simbol atau string. Itu sebabnya bidang ini tidak tersedia untuk rasterisasi dan pertama-tama harus dikonversi menjadi angka. Ini dapat dilakukan dengan mudah dengan kalkulator Bidang:

1. Buka tabel atribut lapisan menggunakan klik kanan Open Shortcut Tabel Atribut, atau tekan tombol relatif dari toolbar Atribut. Di bilah alat tabel atribut, klik tombol Kalkulator bidang terbuka atau gunakan pintasan keyboard Ctrl + I.

2. Pertama-tama, kita harus menyingkirkan nilai X yang tidak dapat diartikan sebagai angka dan tidak dapat dikonversi ke dalamnya. Karena kita hanya memiliki satu baris yang berisi nilai X, kita cukup beralih mode pengeditan dengan mengklik tombol di toolbar tabel atribut. Klik dua kali pada sel dan masukkan nilai baru 7. secara manual. Jika Anda memiliki beberapa nilai untuk diubah, Anda dapat menggunakan ekspresi berikut di Kalkulator bidang untuk mengubah beberapa nilai bidang zona: KASUS KAPAN “zona” = 'X' KEMUDIAN ' 7 'ELSE "zone" END.

3. Klik pada tombol untuk membuka jendela dialog Field calculator seperti yang ditunjukkan pada tangkapan layar berikut, dan lakukan penyesuaian berikut:

- Pastikan bahwa sakelar bidang buat baru diaktifkan.

- Ketikkan nama bidang Output secara manual, misalnya, peringkat.

- Pilih Whole number (integer) dari daftar tipe field Output, karena kita akan menggunakan integer pendek untuk peringkat.

- Kurangi lebar bidang Output ke 1. Ini karena nilai peringkat tidak melebihi 10 dan kita tidak ingin membuat data berlebih dengan menghasilkan bidang panjang yang melebihi nilai data aktual.

- Di bidang Ekspresi, kita perlu mengetik fungsi yang akan digunakan untuk membuat nilai-nilai dari bidang baru. Dalam daftar Fungsi, perluas item Konversi dan klik dua kali pada fungsi toint. Menurut deskripsinya, ini mengonversi string ke angka integer. Tidak ada yang berubah jika nilai tidak dapat dikonversi ke integer (misalnya, 123asd tidak valid). Setelah mengklik dua kali, fungsi akan ditambahkan ke ekspresi dengan braket terbuka, setelah itu Anda harus mengetik (atau mengklik dua kali untuk menambahkan item dari Bidang dan Nilai) nama bidang yang akan dikonversi dalam tanda kutip ganda, dan tutup kurung. Dalam kasus kita, ekspresi yang dihasilkan adalah toint ("zone").

- Setelah mengklik tombol OK, bidang baru akan muncul di akhir tabel. Nonaktifkan mode pengeditan, konfirmasi penyimpanan hasil edit, dan keluar dari jendela tabel atribut.
...
Lapisan siap untuk rasterisasi. Buka jendela dialog dengan pergi ke Raster | Konversi | Rasterize (Vektor ke Raster) . Di jendela dialog ini seperti yang ditunjukkan pada tangkapan layar berikut, sesuaikan pengaturan berikut:

1.Dari daftar drop-down File input (shapefile), pilih hurricane_evacuation_zones.

2. Dari daftar turun bawah Atribut, pilih peringkat.

3. Dalam file Output untuk vektor raster (raster), klik tombol Select. Arahkan ke direktori kerja Anda, dan ketik hurricane_evacuation_zones.tif sebagai nama layer baru. Pesan ini akan ditampilkan: File output tidak ada. Anda harus mengatur ukuran atau resolusi output untuk membuatnya. Klik OK dan lanjutkan ke tahap berikutnya.
...
Ini berarti bahwa raster keluaran akan berisi nilai raster dari bidang atribut peringkat. Area di luar layer poligon akan diberi nilai 0, yang akan ditafsirkan sebagai nodata. Tipe data output adalah bilangan bulat unsigned 16-bit.
...
Dalam kasus raster kepadatan, kita dapat menggunakan ready noise_heatamp.tif dan membuat layer tree_density.tif sendiri. Pertama, kita akan menyiapkan noise_heatmap.tif yang awalnya jauh lebih besar daripada bidang minat yang kita kerjakan dalam bab ini. Untuk klip layer raster pergi ke Raster | Ekstraksi | Clipper :

Dari daftar drop-down File input (raster) pilih noise-heatmap, yang merupakan raster yang akan dipotong.

Dalam file Output, klik tombol Pilih untuk mengatur jalur dan nama untuk file output, misalnya, noise_heatmap_clip.tif.

Di bagian Mode kliping, ada dua mode yang mungkin:
...
Jika layer Mask aktif, Anda dapat memilih bentuk poligonal, dan itu akan digunakan sebagai batas kliping.
